// api/send-report.js — Vercel Serverless Function (CommonJS)
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    page.drawText('ReliableTrack - Field Report', { x: 20, y: y - 30, size: 16, font: bold, color: white })
    y -= 55
    ...
    const descLines = []
    for (let start = 0; start < Math.max(desc.length, 1); start += 90) {
    ...
    const htmlBody = '<h2 style="color:#1a2332">ReliableTrack Field Report</h2>' +

  ...
  try {
    ...
    const drawText = (text, x, yPos, opts = {}) => {
      ...
      page.drawText(String(text || ''), {
        x, y: yPos, font, size, color,
        maxWidth: maxWidth || (marginR - x + 10)
      })
    }
    ...
    // ─── HEADER with ROS Logo ─────────────────────────────────────────────────
    page.drawRectangle({ x: 0, y: pageH - 70, width: pageW, height: 70, color: rosNavy })

    // ROS Logo: draw circular badge on left side of header
    const logoX = 52, logoY = pageH - 35
    const logoR = 28
    // Outer ring
    page.drawCircle({ x: logoX, y: logoY, size: logoR, color: white })
    page.drawCircle({ x: logoX, y: logoY, size: logoR - 3, color: rosNavy })
    // Three vertical bars (like the ROS logo pillars)
    ...
    const partsHtml = parts.length > 0
      ? '<table border="1" cellpadding="4" style="border-collapse:collapse;font-size:12px;margin:8px 0;width:100%"><tr style="background:#1a2332;color:#fff"><th>SKU</th><th>Part</th><th>Qty</th><th>Unit $</th><th>Total</th></tr>' +
...
<div style="background:#1a2332;padding:20px 24px;display:flex;align-items:center">
  <div>
...
<p style="font-size:10px;color:#aaa;padding:8px 24px">Sent by ReliableTrack - Built for Reliable Oilfield Services<br>PDF work order attached.</p>
    ...
    // ─── Email subject: GoCanvas-style format ─────────────────────────────────
    // Format: Customer - Location - Techs - TypeOfWork - ROS PM/SC Work Order

...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const pPhotosList = allPhotos.filter(p => p.section && p.section.startsWith('part-'))
    if (pPhotosList.length > 0) { section('PART PHOTOS'); await addPhotos(pPhotosList) }

    // PM equipment photos: arrestors, flares, heater treaters
    if (jobType === 'PM') {
    ...
    const partsHtml = parts.length > 0
      ? '<table border="1" cellpadding="4" style="border-collapse:collapse;font-size:12px;margin:8px 0;width:100%">'
        ...
        + parts.map(p =>
            '<tr><td>' + (p.sku || p.code || '') + '</td><td>' + (p.name || '') + '</td><td>' + (p.qty || 1) + '</td><td>$'
            + Number(p.price || 0).toFixed(2) + '</td><td>$' + (Number(p.qty || 1) * Number(p.price || 0)).toFixed(2) + '</td></tr>'
          ).join('')
        + '</table>'
      ...
      if (flares.length > 0) {
        pmEquipHtml += '<div style="margin-top:12px"><strong style="color:#1a2332">Flares</strong>'
          ...
          + flares.map((f, i) =>
              ...
              + '</td><td>' + (f.pilotLit ? 'Yes' : 'No') + '</td><td>' + (f.lastIgnition || '') + '</td><td>' + (f.notes || '') + '</td></tr>'
            ).join('')
          + '</table></div>'
      }
      if (heaters.length > 0) {

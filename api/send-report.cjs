...
module.exports = async function handler(req, res) {
  ...
  // Use userToken for RLS-aware queries if provided, else fall back to anon key
  ...
  try {
    ...
    if (!rows || rows.length === 0) return res.status(404).json({ error: 'Submission not found. Check userToken/RLS.' })
    const s = rows[0]
    ...
    const emailRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        attachments: [{
          filename: 'ROS-' + jobLabel.replace(/ /g,'-').replace('#','') + '-' + customer.replace(/[^a-zA-Z0-9]/g,'-') + '.pdf',
          content: pdfBase64
        }]
      })
    })

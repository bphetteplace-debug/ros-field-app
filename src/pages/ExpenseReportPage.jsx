  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      try {
        ...
        fetch('/api/send-report', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ submissionId: submission.id, userToken: token }) }).then(async r => { if (!r.ok) { const t = await r.text().catch(() => r.statusText || ''); throw new Error('HTTP ' + r.status + ' — ' + (t || '').slice(0, 240)); } }).catch(err => { console.error('Email send failed:', err); toast.error('Email failed for expense report #' + (submission.pm_number || submission.id) + '\n\n' + (err.message || err) + '\n\nThe report was saved. Open it from the list and use "Send Report" to retry.'); })
      } catch (_) {}
      await clearDraft()

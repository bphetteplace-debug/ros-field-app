...
export default function SubmissionsListPage() {
  ...
  // Resolve the user's first name with safe fallbacks so we never render "undefined".
  const firstName =
  ...
  return (
    <main className="max-w-5xl mx-auto p-3 sm:p-6">
      {/* Header with title + the two action buttons (the only place they live now) */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-2">
          <Link
            to="/form?type=pm"
            className="px-4 py-2.5 bg-orange-600 hover:bg-orange-700 text-white font-bold rounded-md flex items-center gap-2 transition shadow flex-1 sm:flex-none justify-center"
          >
            <Plus className="w-4 h-4" /> PM
          </Link>
          <Link
            to="/form?type=sc"
            className="px-4 py-2.5 bg-slate-700 hover:bg-slate-800 text-white font-bold rounded-md flex items-center gap-2 transition shadow flex-1 sm:flex-none justify-center whitespace-nowrap"
          >
      ...
      {/* Empty state — text only, no duplicate buttons */}
      <div className="bg-white rounded-lg shadow-sm p-12 text-center">
        ...
        <p className="text-sm text-slate-500 max-w-md mx-auto">
          {isCloudMode
            ? 'Tap PM or Service Call above to start your first field report. Saved submissions will appear here.'
